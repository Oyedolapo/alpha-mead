import { Header, Footer, InterBusinessHero, Breadcrumb } from "../components";
import { useNavigate } from "react-router-dom";

const ServiceLayout = ({ heroText, heroImage, title, paragraphs, cards, caseStudy, caseLink, training, registerBtn, breadcrumbItems, cardText }) => {
        ...
        <section className="font-roboto bg-white min-h-screen pt-[7rem]">
            ...
            <main className="max-w-6xl mx-auto px-6 lg:px-10 py-16">
                ...
                <div className="flex flex-col lg:flex-row items-start gap-10">
                    ...
                    <div className="w-full lg:w-1/2">
                        <img
                            src={heroImage}
                            alt={title}
                            className="w-full lg:h-[300px] object-cover rounded-2xl shadow-md"
                        />
                        {caseStudy && (
                            <button
                    ...
                    <div className="w-full lg:w-1/2 text-gray-700">
                        ...
                        {/* Register Button */}
                        {registerBtn && (
                            <button className="bg-[#71bf4f] hover:bg-green-600 uppercase text-white px-8 py-3 text-sm rounded-2xl transition-colors">
                                Register Here
                            </button>
                        )}
                    </div>
                ...
                {training && (
                    <div className="mt-10">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-3">
                            <h3 className="text-2xl font-semibold text-gray-900">Our Courses</h3>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4"> 
                                <button className="bg-[#71bf4f] hover:bg-green-600 uppercase text-white px-8 py-3 text-sm rounded-2xl transition-colors w-full sm:w-auto">
                                    <a href="https://alphamead.com/wp-content/uploads/2025/08/AMTC-CALENDER-2025.pdf" target="_blank" rel="noopener noreferrer">
                                        Download Calendar
                                    </a>
                                </button>
                                <button className="bg-[#71bf4f] hover:bg-green-600 uppercase text-white px-8 py-3 text-sm rounded-2xl transition-colors w-full sm:w-auto">
                ...
                {cards && (
                    <>
                        {cardText && (
                            <h3 className="text-xl font-semibold text-gray-900 pt-10 pb-4">{cardText}</h3>
                        )}
